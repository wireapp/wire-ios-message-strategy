...
extension AssetV3ImageUploadRequestStrategy: ZMUpstreamTranscoder {
    ...
    public func request(forUpdating managedObject: ZMManagedObject, forKeys keys: Set<String>) -> ZMUpstreamRequest? {
        ...
        guard let request = requestFactory.upstreamRequestForAsset(withData: data, shareable: false, retention: .persistent) else { fatal("Could not create asset request with data \(data)") }
