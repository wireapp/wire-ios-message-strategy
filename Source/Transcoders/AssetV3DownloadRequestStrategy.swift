...
@objc public final class AssetV3DownloadRequestStrategy: AbstractRequestStrategy, ZMDownstreamTranscoder, ZMContextChangeTrackerSource {
    ...
    private func storeAndDecrypt(data: Data, for message: ZMAssetClientMessage, fileMessageData: ZMFileMessageData) -> Bool {
        ...
        if asset.original.hasImage() && asset.original.mimeType != "image/svg+xml" {
    ...
    private func storeAndDecryptImage(asset: ZMAsset, nonce: UUID, data: Data, keys: DecryptionKeys) -> Bool {
        precondition(asset.original.hasImage() && asset.original.mimeType != "image/svg+xml", "Should only be called for assets with image")
    ...
    private func storeAndDecryptFile(asset: ZMAsset, nonce: UUID, data: Data, keys: DecryptionKeys, name: String) -> Bool {
        precondition(!asset.original.hasImage() || asset.original.mimeType == "image/svg+xml", "Should not be called for assets with image")
